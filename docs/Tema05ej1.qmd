...
### Datos económicos de FRED, 
  ...
  + En la web de FRED, se busca un dato concreto:  por ejemplo, inflación en España que aparece como "Inflation, consumer prices for Spain; Percent, Annual, Not Seasonally Adjusted"
...
Esta biblioteca también incluye funciones de análisis específicos, incluyendo `geoms` de `ggplot2` para nuevos tipos de gráficos. Podemos representar los distintos precios de una o de varias empresas ("símbolos") a la vez.
...
* Para la serie financiera, 
  ...
  a. Realizar un gráfico de barras para **todo** el periodo incluyendo información sobre las tendencias mediante medias móviles. Comentar brevemente. 
  
  b. En base al gráfico anterior, elegid un periodo de 6 meses (que NO incluya la segunda mitad de 2024) que consideréis relevante para realizar un gráfico de velas. Realizar un breve análisis descriptivo para este periodo. Comentar brevemente.
  
* Para la serie económica, 

  a. Realizar un gráfico con su evolución temporal en **todo** el periodo. Comentar brevemente.
  
  b. En base al gráfico anterior, elegid dos periodos diferentes de 5 años (que NO incluyan la segunda mitad de 2024). Realizar un breve análisis descriptivo para estos dos periodos. Comentar brevemente resaltando las diferencias y similitudes entre los periodos.
...
##  Interest Rates: Long-Term Government Bond Yields: 
## 10-Year: Main (Including Benchmark) for Spain (IRLTLT01ESA156N)
tipos  <- tq_get("IRLTLT01ESA156N", get = "economic.data", from = "1960-01-01")
...
Apple %>% 
  filter(year(date)>2023 & month(date)>4) %>%  
  ggplot(aes(x = date, y = close)) +
    geom_candlestick(aes(open = open, high = high, low = low, close = close))
