...
-  Desde el menú *File > New Proyect* o desde el icono, <!-- ![](figure/newProject.png){width="20%" .center} --> creamos un nuevo proyecto que estará asociado a un directorio de trabajo. 
...
* O bien elegimos un directorio ya existente para nuestro proyecto
  ...
  <!--  ![](figure/newProjectDir.png){width="20%" .center} -->
...
* La **pestaña** ![](figure/Files.jpg){width="8%"} en el cuadrante inferior-derecho ofrece una forma visual de <!-- realizar operaciones de archivos: --> abrir, crear, copiar, mover o eliminar archivos o directorios, etc.
...
-   En RStudio, la pestaña ![](figure/Environment.jpg){width="15%"} muestra los objetos
    y su valor
...
-  Guardamos el contenido del entorno de trabajo con ![](figure/guardar.png){width="5%" .center} <!--en el `Environment`--> (o al cerrar RStudio), pero es innecesario si tenemos la secuencia de comandos
...
-   Un vector es una secuencia de <!--objetos--> datos elementales, creados con el operador "`c()`" (combinar)
...
- Si se mezclan <!--objetos elementales de clases distintas-->tipos distintos, R busca una clase que "acomode" a todos
...
length(xvect)   # longitud: número de elementos
sort(xvect)     # ordenar
 
max(xvect)      # máximo
min(xvect)      # mínimos
sum(xvect)      # suma de los elementos
prod(xvect)     # producto de los elementos

mean(xvect)     # media de los ementos
var(xvect)      # varianza
table(xvect)    # tabla de frecuencias

summary(xvect)  # estadísticos descriptivos
```
-->
...
<!-- a veces es el comportamiento deseado -->
...
-   Obtenemos objeto lógico  enunciando una relación<!--(con operadores lógicos básicos)--> que puede ser cierta <!--(`TRUE`)--> o falsa <!--(`FALSE`)-->
...
2. **Nombres** de los elementos, usando un vector de caracteres
...
3. **Condición** que satisfacen los elementos, usando un vector lógico
```{r}
#| echo: false
altura[altura >= 165]
...
-  Se pueden crear con `matrix()` o <!--También se crear -->uniendo vectores filas o vectores columnas (de las mismas dimensiones) con `cbind()` y `rbind()`
...
-   Se pueden hacer todo tipo [operaciones matriciales con
    R](http://www.statmethods.net/advstats/matrix.html) como multiplicación matricial, `%*%`, transponer, `t(M1)`, invertir, `solve(A)`, etc.
...
-   Descargad estos ejemplos: [renta.txt](https://albarran.quarto.pub/big-data-economia-ua/docs/data/renta.txt), [sex_data.csv](https://albarran.quarto.pub/big-data-economia-ua/docs/data/sex_data.csv), [beauty.xls](https://albarran.quarto.pub/big-data-economia-ua/docs/data/beauty.xls), [nsw.dta](https://albarran.quarto.pub/big-data-economia-ua/docs/data/nsw.dta)
...
-   Bibliotecas con funciones para acceder directamente a datos online
    (con
...
-   Muchos conjuntos de datos tienen valores ausentes de ciertas observaciones para algunas variables: ej., descargad [earn.RData](https://albarran.quarto.pub/big-data-economia-ua/docs/data/earn.RData)
